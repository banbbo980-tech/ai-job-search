...
6. Fetch posting content for each candidate. Never score from title alone.
...
11. Apply verdict bands from the evaluation framework:
    ...
    - Weak Fit: 30-44
    - Poor Fit: below 30
12. Apply location deal-breakers as vetoes. A high-scoring job with location
    `FAIL` is excluded, not ranked first.
13. Mark past-deadline jobs expired. Deadlines within seven days win ties and
    receive an urgency note.
14. Update `job_scraper/seen_jobs.json` additively:
    - Ranked jobs: `status`, `rank_score`, `rank_verdict`, `rank_date`.
    - Expired jobs: `status: "expired"`.
    - Do not restructure existing entries.
15. Present shortlist, below-threshold jobs, and excluded/expired jobs. Then ask
